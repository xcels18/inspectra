        ...
        <div class="card-body py-3 px-4 position-relative z-1">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex align-items-center gap-1 px-3 py-1 rounded-pill" style="background:rgba(251,191,36,0.2); font-size:0.75rem; color:#fde68a; border:1px solid rgba(251,191,36,0.3);">
                        <i class="bi bi-cloud-slash me-1"></i>{{ $belumSync }} belum
                    </div>
                    
                    {{-- Overall progress bar --}}
                    @if($totalDokumen > 0)
                    <div style="width:100px; margin: 0 0.5rem;" class="d-none d-md-block">
                        @php $overallPct = round($sudahSync / $totalDokumen * 100); @endphp
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            <span style="font-size:0.65rem; color:rgba(255,255,255,0.7);">Progress</span>
                            <span style="font-size:0.65rem; color:rgba(255,255,255,0.9); font-weight:600;">{{ $overallPct }}%</span>
                        </div>
                        <div style="height:5px; background:rgba(255,255,255,0.15); border-radius:4px; overflow:hidden;">
                            <div style="height:100%; width:{{ $overallPct }}%; background:#10b981; border-radius:4px; transition:width 0.4s;"></div>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="d-flex gap-1 ms-1">
                        <button class="btn btn-sm fw-semibold shadow-sm" id="btnSyncAll"
                                style="background:#fff; color:#0b192c; font-size:0.78rem; transition:transform 0.15s;"
                        ...
                        <form id="resetAllSyncForm" action="{{ route('google-drive.reset-sync') }}" method="POST" class="d-inline">
                            @csrf
                            <button type="button" class="btn btn-sm fw-semibold shadow-sm"
                                    style="background:rgba(239,68,68,0.2); color:#fca5a5; border:1px solid rgba(239,68,68,0.3); font-size:0.78rem;"
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-header py-2 px-3 d-flex align-items-center justify-content-between" style="background:#f8f9fa; border-bottom:1px solid #e9ecef;">
            <span class="fw-semibold text-muted" style="font-size:0.82rem;">
                <i class="bi bi-folder-fill me-1 text-warning"></i>Daftar Pemeriksaan
                <span class="badge bg-secondary ms-1" style="font-size:0.68rem;">{{ $pemeriksaanStatsPaginated->total() }}</span>
            </span>
            <span class="text-muted" style="font-size:0.75rem;">
                Halaman {{ $pemeriksaanStatsPaginated->currentPage() }} dari {{ $pemeriksaanStatsPaginated->lastPage() }}
            </span>
        </div>
        <div class="card-body p-0">
            @forelse($pemeriksaanStatsPaginated as $pStat)
            @php
                $pemeriksaan = $pStat['pemeriksaan'];
                $pTotal      = $pStat['total'];
                $pSynced     = $pStat['synced'];
                $pUnsynced   = $pStat['unsynced'];
                $pPct        = $pTotal > 0 ? round($pSynced / $pTotal * 100) : 0;
            @endphp

            {{-- Baris Pemeriksaan --}}
            <div class="border-bottom pemeriksaan-row" id="rowPem{{ $pemeriksaan->id }}" style="background: #fafafa;">
                <div class="d-flex align-items-center gap-2 px-3 py-2"
                     style="cursor:pointer; transition:background 0.15s;"
                     onclick="togglePemeriksaan({{ $pemeriksaan->id }})"
                     onmouseenter="this.style.background='#f0f2f5'"
                     onmouseleave="this.style.background=''">
                    
                    <span id="toggleIconPem{{ $pemeriksaan->id }}" class="text-muted" style="font-size:0.75rem; min-width:14px;">
                        <i class="bi bi-chevron-right"></i>
                    </span>

                    <div style="min-width:0; flex:1;">
                        <div class="d-flex align-items-center gap-1 flex-wrap">
                            <span class="fw-bold" style="font-size:0.85rem; color:#1f2937;">{{ $pemeriksaan->nama }}</span>
                            @if($pemeriksaan->gdrive_folder_id)
                                <a href="https://drive.google.com/drive/folders/{{ $pemeriksaan->gdrive_folder_id }}"
                        ...
                        <div class="text-muted text-truncate" style="font-size:0.75rem;">Tahun: {{ $pemeriksaan->tahun }}</div>
                    </div>

                    {{-- Progress Pemeriksaan --}}
                    <div style="width:100px; flex-shrink:0;">
                        <div class="d-flex justify-content-between" style="font-size:0.68rem;">
                            <span class="{{ $pPct == 100 ? 'text-success' : 'text-muted' }}">{{ $pSynced }}/{{ $pTotal }}</span>
                            <span class="fw-semibold {{ $pPct == 100 ? 'text-success' : 'text-primary' }}">{{ $pPct }}%</span>
                        </div>
                        <div style="height:4px; background:#e9ecef; border-radius:4px; overflow:hidden;">
                            <div style="height:100%; width:{{ $pPct }}%; background:{{ $pPct == 100 ? '#22c55e' : '#3b82f6' }}; border-radius:4px;"></div>
                        </div>
                    </div>
                </div>

                {{-- Isi Pemeriksaan (Daftar Surat) --}}
                <div id="pemeriksaanList{{ $pemeriksaan->id }}" style="display:none; padding-left: 20px;">
                    @foreach($pStat['surat_stats'] as $stat)
                    @php
                        $surat    = $stat['surat'];
                        $total    = $stat['total'];
                        $synced   = $stat['synced'];
                    ...
                    {{-- Baris surat --}}
                    <div class="border-bottom surat-row bg-white" id="row{{ $surat->id }}">
                        ...
                        <div class="d-flex align-items-center gap-2 px-3 py-2"
                            ...
                            {{-- Struktur folder --}}
                            @php
                                $structAfterNomor = array_values(array_filter($struct, fn($l) => !in_array($l, ['nomor_surat', 'pemeriksaan'])));
                            ...
                            <div class="d-none d-lg-flex align-items-center gap-1">
                                ...
                                <span style="font-size:0.6rem; font-weight:600; background:#f59e0b; color:#fff; padding:1px 6px; border-radius:3px;">Pemeriksaan</span>
                                <i class="bi bi-chevron-right text-muted" style="font-size:0.45rem;"></i>
                        ...
                        <div id="dokumenList{{ $surat->id }}" style="display:none;">
                            @if($total > 0)
                            <div class="table-responsive" style="background:#fdfdfe; border-top:1px solid #f0f0f0; overflow-x: auto;">
                                <table class="table table-hover align-middle mb-0" style="font-size:0.8rem; border-collapse: separate; border-spacing: 0;">
                                    <thead style="background:#f8f9fa;">
                                        <tr>
                                            <th class="ps-4 border-bottom-0 py-2" style="width:40px; color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">#</th>
                                            <th class="border-bottom-0 py-2" style="color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">Nama File</th>
                                            <th class="d-none d-md-table-cell border-bottom-0 py-2" style="color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">OPD</th>
                                            <th class="d-none d-lg-table-cell border-bottom-0 py-2" style="color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">Permintaan</th>
                                            <th class="border-bottom-0 py-2" style="width:100px; color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">Status</th>
                                            <th class="d-none d-sm-table-cell border-bottom-0 py-2" style="width:130px; color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">Waktu Sync</th>
                                            <th class="border-bottom-0 py-2" style="width:80px; color:#6c757d; font-weight:600; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.5px;">Aksi</th>
                                        </tr>
                                    </thead>
                                        ...
                                        @foreach($surat->judulPermintaan as $judul)
                                            @foreach($judul->permintaanData as $perm)
                                                @foreach($perm->permintaanOpd as $opd)
                                                    ...
                                                    <tr class="{{ $dok->gdrive_synced_at ? '' : 'table-warning' }}">
                                                        <td class="ps-4 text-muted fw-medium py-2">{{ $no++ }}</td>
                                                        <td class="py-2">
                                                            <div class="d-flex align-items-center">
                                                                <i class="bi bi-file-earmark-text fs-6 {{ $dok->gdrive_synced_at ? 'text-primary' : 'text-warning' }} me-2"></i>
                                                                <span class="text-truncate fw-medium text-dark" style="max-width:250px;" title="{{ $dok->nama_file }}">{{ $dok->nama_file }}</span>
                                                            </div>
                                                        </td>
                                                        <td class="text-secondary d-none d-md-table-cell py-2" style="font-size:0.75rem;">
                                                            <span class="badge bg-light text-secondary border fw-normal text-wrap text-start" style="line-height:1.2;">{{ $opd->opd }}</span>
                                                        </td>
                                                        <td class="text-secondary d-none d-lg-table-cell py-2" style="font-size:0.75rem; max-width:200px;">
                                                            <div class="text-truncate" title="{{ $perm->judul_permintaan }}">
                                                                {{ $perm->judul_permintaan }}
                                                            </div>
                                                        </td>
                                                        <td class="py-2">
                                                            @if($dok->gdrive_synced_at)
                                                                <span class="badge bg-success-subtle text-success border border-success-subtle px-2 py-1" style="font-weight:500;">
                                                                    <i class="bi bi-check-circle me-1"></i>Tersinkron
                                                                </span>
                                                            @else
                                                                <span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2 py-1" style="font-weight:500;">
                                                                    <i class="bi bi-clock-history me-1"></i>Belum
                                                                </span>
                                                            @endif
                                                        </td>
                                                        <td class="text-secondary d-none d-sm-table-cell py-2" style="font-size:0.75rem;">
                                                            {{ $dok->gdrive_synced_at ? $dok->gdrive_synced_at->format('d/m/Y H:i') : '-' }}
                                                        </td>
                                                        <td class="py-2">
                                                            @if(!$dok->gdrive_synced_at)
                                                            <form action="{{ route('dokumen.destroy', $dok) }}" method="POST" class="d-inline"
                                                                  onsubmit="return confirm('Hapus dokumen ini dari daftar? File lokal juga akan ikut terhapus.')">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button type="submit" class="btn btn-sm btn-outline-danger py-1 px-2 border-0" title="Hapus dokumen" style="background-color:rgba(220,53,69,0.1);">
                                ...
                                <form action="{{ route('google-drive.set-structure', $surat->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <p class="text-muted mb-2" style="font-size:0.82rem;">
                                            Susunan folder dimulai dari <strong>ROOT / Pemeriksaan / Nomor Surat</strong>, kemudian pilih level berikutnya:
                                        </p>
                                        <div class="d-flex align-items-center gap-1 mb-3 flex-wrap" style="font-size:0.8rem;">
                                            ...
                                            <span class="badge bg-warning px-2 py-1 text-dark">Pemeriksaan</span>
                                            <i class="bi bi-chevron-right text-muted"></i>
                                        ...
                                        <div id="sortable{{ $surat->id }}" class="border rounded p-2" style="min-height:60px; background:#fafafa;">
                                            @php
                                                ...
                                                $currentStruct = array_filter($stat['structure'], fn($l) => !in_array($l, ['nomor_surat', 'pemeriksaan']));
                                            ...
                                            <div class="d-flex align-items-center gap-2 p-2 mb-1 bg-white border rounded drag-item"
                                                ...
                                                <label class="form-check-label mb-0"
                                                    ...
                                                    {{ $allLevels[$lvlKey] ?? ucfirst(str_replace('_', ' ', $lvlKey)) }}
                                                </label>
                                            </div>
                                            @endforeach
                                        </div>
                    ...
                    @endforeach
                    @if(count($pStat['surat_stats']) == 0)
                    <div class="px-4 py-2 text-muted" style="font-size:0.78rem;">
                        <i class="bi bi-inbox me-1"></i>Belum ada surat di pemeriksaan ini.
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="text-center py-5 text-muted">
                ...
                <div class="mt-2" style="font-size:0.85rem;">Belum ada Pemeriksaan.</div>
            </div>
            @endforelse
        </div>

        {{-- Pagination Pemeriksaan --}}
        @if($pemeriksaanStatsPaginated->hasPages())
        <div class="card-footer py-2 px-3 d-flex align-items-center justify-content-between" style="background:#f8f9fa;">
            <small class="text-muted" style="font-size:0.75rem;">
                Menampilkan {{ $pemeriksaanStatsPaginated->firstItem() }}–{{ $pemeriksaanStatsPaginated->lastItem() }}
                dari {{ $pemeriksaanStatsPaginated->total() }} pemeriksaan
            </small>
            <div>
                {{ $pemeriksaanStatsPaginated->links('pagination::bootstrap-5') }}
            </div>
        </div>
        @endif
    </div>

    {{-- Tabel Belum Dipetakan --}}
    @if(count($unmappedStats['surat_stats']) > 0)
    <div class="card shadow-sm border-0 mt-4">
        <div class="card-header py-2 px-3 d-flex align-items-center justify-content-between" style="background:#fff3cd; border-bottom:1px solid #ffe69c;">
            <span class="fw-semibold text-dark" style="font-size:0.82rem;">
                <i class="bi bi-exclamation-triangle-fill me-1 text-warning"></i>Surat Belum Dipetakan (Tanpa Pemeriksaan)
                <span class="badge bg-warning text-dark ms-1" style="font-size:0.68rem;">{{ count($unmappedStats['surat_stats']) }}</span>
            </span>
        </div>
        <div class="card-body p-0">
            @foreach($unmappedStats['surat_stats'] as $stat)
                @php
                    $surat    = $stat['surat'];
                    $total    = $stat['total'];
                    $synced   = $stat['synced'];
                    ...
                    $pct      = $total > 0 ? round($synced / $total * 100) : 0;
                @endphp
                {{-- COPY THE SURAT ROW HERE WITH MINOR ADJUSTMENTS --}}
                <div class="border-bottom surat-row bg-white" id="row{{ $surat->id }}">
                        ...
                        <div class="d-flex align-items-center gap-2 px-3 py-2"
                            ...
                            <div class="d-none d-lg-flex align-items-center gap-1">
                                <span style="font-size:0.6rem; font-weight:600; background:#4b5563; color:#fff; padding:1px 6px; border-radius:3px;">ROOT</span>
                                <i class="bi bi-chevron-right text-muted" style="font-size:0.45rem;"></i>
                                <span style="font-size:0.6rem; font-weight:600; background:#f59e0b; color:#fff; padding:1px 6px; border-radius:3px;">Belum Dipetakan</span>
                                <i class="bi bi-chevron-right text-muted" style="font-size:0.45rem;"></i>
                                <span style="font-size:0.6rem; font-weight:600; background:#1a73e8; color:#fff; padding:1px 6px; border-radius:3px;">No.Surat</span>
                                @foreach($structAfterNomor as $lvl)
                                    <i class="bi bi-chevron-right text-muted" style="font-size:0.45rem;"></i>
                        ...
                        <div class="modal-dialog modal-md">
                            <div class="modal-content">
                                <form action="{{ route('google-drive.set-structure', $surat->id) }}" method="POST">
                                    ...
                                    <div class="modal-body">
                                        <p class="text-muted mb-2" style="font-size:0.82rem;">
                                            Susunan folder dimulai dari <strong>ROOT / Belum Dipetakan / Nomor Surat</strong>, kemudian pilih level berikutnya:
                                        </p>
                                        <div class="d-flex align-items-center gap-1 mb-3 flex-wrap" style="font-size:0.8rem;">
                                            <span class="badge bg-secondary px-2 py-1">ROOT</span>
                                            <i class="bi bi-chevron-right text-muted"></i>
                                            <span class="badge bg-warning px-2 py-1 text-dark">Belum Dipetakan</span>
                                            <i class="bi bi-chevron-right text-muted"></i>
                                            <span class="badge bg-primary px-2 py-1">Nomor Surat</span>
                                            <i class="bi bi-chevron-right text-muted" id="arrow{{ $surat->id }}"></i>
                                            <span id="previewLevels{{ $surat->id }}" class="d-flex gap-1 flex-wrap"></span>
...
// Toggle collapse surat per pemeriksaan
function togglePemeriksaan(pemId) {
    const list     = document.getElementById('pemeriksaanList' + pemId);
    const icon     = document.getElementById('toggleIconPem' + pemId);
    const expanded = list.style.display !== 'none';
    list.style.display  = expanded ? 'none' : 'block';
    icon.innerHTML      = expanded
        ? '<i class="bi bi-chevron-right"></i>'
...
document.querySelectorAll('[id^="sortable"]').forEach(function(container) {
    const suratId = container.id.replace('sortable', '');
