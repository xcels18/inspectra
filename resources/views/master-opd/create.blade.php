...
@section('page-title', 'Tambah Master OPD')
        ...
        <div class="card-body py-3 px-4 d-flex align-items-center justify-content-between flex-wrap gap-2 position-relative z-1">
            <div>
                <h5 class="mb-0 fw-bold text-white d-flex align-items-center gap-2">
                    <i class="bi bi-building-add"></i> Tambah Master OPD
                </h5>
                <div style="font-size:0.75rem; color:#94a3b8; margin-top:2px;">Buat data OPD baru</div>
            </div>
            ...
            <div class="card-body pt-3 pb-4">
                <div class="row g-3">
                    <div class="col-md-8">
                        ...
                        <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama') }}" placeholder="Contoh: DINAS PENDIDIKAN DAN KEBUDAYAAN" required>
                        @error('nama')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Kategori <span class="text-danger">*</span></label>
                        <select name="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                            <option value="OPD" {{ old('kategori') == 'OPD' ? 'selected' : '' }}>OPD</option>
                            <option value="Sekolah" {{ old('kategori') == 'Sekolah' ? 'selected' : '' }}>Sekolah</option>
                            <option value="Partai Politik" {{ old('kategori') == 'Partai Politik' ? 'selected' : '' }}>Partai Politik</option>
                            <option value="Instansi Vertical" {{ old('kategori') == 'Instansi Vertical' ? 'selected' : '' }}>Instansi Vertical</option>
                        </select>
